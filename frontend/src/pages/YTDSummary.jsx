...
export default function YTDSummary() {
  const now = new Date()
  const defaultFyStart = now.getMonth() + 1 >= 4 ? now.getFullYear() : now.getFullYear() - 1
  const [fyYear, setFyYear] = useState(defaultFyStart)
  ...
  useEffect(() => {
    ...
    setError('')
    api
      .get('/api/reports/ytd', { params: { year: fyYear } })
  ...
  const fyLabel = data.fy_label || `${fyYear}-${(fyYear + 1) % 100}`
