...
export default function AllFY() {
  ...
  useEffect(() => {
    ...
    setError('')
    api
      .get('/api/reports/all-fy')
      .then((res) => setData(res.data))
      .catch((err) =>
        setError(err.response?.data?.detail || 'Failed to load All FY')
      )
  ...
  // Column values: totals for each FY
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">All Financial Years</h1>
        <p className="text-slate-500 text-sm">
          Every Financial Year since data exists, side by side
        </p>
      </div>
