...
def _expense_by_nature(db, branch_id, month=None, year=None):
    """Return {'Fixed': x, 'Variable': y} totals based on ledger.nature."""
    q = (
        db.query(
...
def _pnl(db, branch_id, month=None, year=None):
    ...
    return {
        ...
        "gross_revenue": gross,
        "net_revenue": net,
        "total_billing": net + rev["gst"],
        "expenses": exp,
        "total_expenses": total_exp,
        "total_fixed_expenses": nat["Fixed"],
        "total_variable_expenses": nat["Variable"],
        "net_profit": net - total_exp,
        "occupancy_pct": (
            (rev["rooms_occupied"] / rev["rooms_available"] * 100)
            if rev["rooms_available"]
            else 0
        ),
        "arr": (
            (rev["room_revenue"] / rev["rooms_occupied"])
            if rev["rooms_occupied"]
            else 0
        ),
    }
...
def ytd(year: int = Query(...), db: Session = Depends(get_db)):
    ...
    for b in branches:
        ...
        totals["total_fixed_expenses"] += pnl["total_fixed_expenses"]
        totals["total_variable_expenses"] += pnl["total_variable_expenses"]
...
def comparison(
    ...
    return {
        ...
        "variance": {
            ...
            "total_fixed_expenses": pct(
                a["total_fixed_expenses"], b["total_fixed_expenses"]
            ),
            "total_variable_expenses": pct(
                a["total_variable_expenses"], b["total_variable_expenses"]
            ),
